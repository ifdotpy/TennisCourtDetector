...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, pad=1, stride=1, bias=True, depthwise=False):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size, stride=stride, padding=pad, groups=in_channels,
                      bias=bias),
...
class CourtFinderNet(nn.Module):
...
if __name__ == '__main__':
    device = 'cpu'
    model = CourtFinderNet().to(device)
